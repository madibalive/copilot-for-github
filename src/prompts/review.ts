import type { LearnedPrefs } from "../types.js";

export function buildTeamContextBlock(teamContext: string | null): string {
  if (!teamContext?.trim()) return "";
  return `## Team Context\n\n${teamContext.trim()}`;
}

/**
 * Builds a "Learned Team Preferences" block from merged learned prefs.
 * suppressions  → comments the team has 👎'd (score < 0)
 * amplifications → comments the team has 👍'd (score > 0)
 * Returns empty string when there are no signals to inject.
 */
export function buildLearnedPrefsPrompt(prefs: LearnedPrefs | null): string {
  const suppress = prefs?.suppressions ?? [];
  const amplify = prefs?.amplifications ?? [];
  if (suppress.length === 0 && amplify.length === 0) return "";

  const lines: string[] = ["## Learned Team Preferences", ""];
  lines.push("The following is based on team reactions (👍/👎) to previous review comments.");
  lines.push("");
  if (suppress.length > 0) {
    lines.push("SUPPRESS (team has 👎'd these repeatedly — reduce or omit similar comments):");
    for (const s of suppress) {
      lines.push(`- ${s.pattern} (score: ${s.score})`);
    }
    lines.push("");
  }
  if (amplify.length > 0) {
    lines.push("AMPLIFY (team has 👍'd these — keep making similar comments):");
    for (const a of amplify) {
      lines.push(`- ${a.pattern} (score: +${a.score})`);
    }
    lines.push("");
  }
  return lines.join("\n").trimEnd();
...
const TOOL_DOCS: Record<string, string> = {
  ...
  report_finding:
    "`report_finding({ category, severity, status, title, details?, evidence?, action?, confidence? })` — record every finding for deterministic rendering. Set confidence (0–5): 5=certain bug/security issue, 4=very likely, 3=probable, 2=possible concern, 1=style preference, 0=speculative. Omit for critical/security findings (implies high confidence). Use lower scores for style, naming, or speculative concerns.",
  set_summary_mode:
