...
import { BUILTIN_COMMANDS } from "./commands/builtin.js";
...
async function main(): Promise<void> {
  try {
    ...
    const userDefinedIds = new Set(actionConfig.commands.map((c) => c.id));
    const effectiveCommands = [
      ...BUILTIN_COMMANDS.filter((b) => !userDefinedIds.has(b.id)),
      ...actionConfig.commands,
    ];
    const registry = new CommandRegistry(effectiveCommands);
    const mode = resolveRunMode();
    if (mode.mode === "pull_request") {
      const context = readContext(mode.prNumber);
      await runActionFlow({
        config: actionConfig,
        skipAutomatic: actionConfig.skipAutomatic,
        triggerOnUpdates: actionConfig.triggerOnUpdates,
        prAction: mode.prAction,
        authorFilters: actionConfig.authorFilters,
        keywordFilters: actionConfig.keywordFilters,
        baseBranchFilters: actionConfig.baseBranchFilters,
        headBranchFilters: actionConfig.headBranchFilters,
        labelFilters: actionConfig.labelFilters,
        context,
        octokit,
        logDebug: core.info,
