...
import type { ChangedFile, ExistingComment, PullRequestInfo, ReactionSummary, ReviewContext, ReviewThreadInfo } from "../types.js";
...
/**
 * Fetches 👍/👎 reactions for all bot-posted comments.
 * Only comments containing the bot marker are queried — non-bot comments are skipped.
 * Reactions are fetched in parallel.
 */
export async function fetchReactionsForBotComments(
  ...
  const results = await Promise.allSettled(
    botComments.map(async (comment): Promise<ReactionSummary> => {
      ...
      const thumbsUp = data.filter((r: { content: string }) => r.content === "+1").length;
      const thumbsDown = data.filter((r: { content: string }) => r.content === "-1").length;
      return {
