  ...
  const reportFindingTool: AgentTool<typeof ReportFindingSchema, { count: number }> = {
    ...
    execute: async (_id, params) => {
      ...
      summaryFindings.push({
        ...
        confidence: params.confidence as (0 | 1 | 2 | 3 | 4 | 5) | undefined,
      });
      return {
...
const ReportFindingSchema = Type.Object({
  ...
  confidence: Type.Optional(Type.Integer({
    minimum: 0,
    maximum: 5,
    description: "Confidence score (0–5). 5=certain bug/security, 4=very likely, 3=probable, 2=possible concern, 1=style preference, 0=speculative. Omit for high confidence.",
  })),
});
...
function ensureSummaryFooter(
  ...
  const hasFooter = body.includes("Reviewed by copilot-for-github");
  ...
  const footer = `---\n*Reviewed by copilot-for-github • model: ${modelId}*\n${billingLine}\n${BOT_COMMENT_MARKER}\n${marker}`;
  if (hasFooter) {
