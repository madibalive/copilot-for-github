...
export function buildSummaryMarkdown(content: SummaryContent): string {
  ...
  return `## Review Summary\n\n**Verdict:** ${content.verdict}\n\n### Issues Found\n\n${renderList(content.issues)}\n\n### Key Findings\n\n${renderList(content.keyFindings)}\n${multiFile}\n---\n*Reviewed by copilot-for-github • model: ${content.model}*${billing}${botMarker}${marker}`;
}
...
function confidencePrefix(confidence: StructuredSummaryFinding["confidence"]): string {
  if (confidence === undefined || confidence >= 4) return "";
  if (confidence === 3) return "(~) ";
  if (confidence === 0) return "(speculative) ";
  return "(?) ";
}

function renderFindingLine(finding: StructuredSummaryFinding, verbose: boolean): string {
  const prefix = confidencePrefix(finding.confidence);
  if (!verbose) {
    return `${prefix}[${finding.severity}] ${finding.title}`;
  }
  const parts = [`${prefix}[${finding.severity}] ${finding.title}`];
