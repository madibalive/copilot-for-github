...
test("resolveRunModeFromEvent exposes prAction for pull_request events", () => {
  const mode = resolveRunModeFromEvent("pull_request", { pull_request: { number: 1 }, action: "synchronize" });
  expect(mode.mode).toBe("pull_request");
  if (mode.mode === "pull_request") {
    expect(mode.prAction).toBe("synchronize");
  }
});

test("resolveRunModeFromEvent exposes prAction as empty string when missing", () => {
  const mode = resolveRunModeFromEvent("pull_request", { pull_request: { number: 1 } });
  if (mode.mode === "pull_request") {
    expect(mode.prAction).toBe("");
  }
});
